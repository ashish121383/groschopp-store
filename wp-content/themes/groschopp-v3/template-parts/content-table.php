...
<?php //if($products = get_products($args, $products_per_page, $offset)): ?>
<?php

    if(get_query_var('s')) {
	    $products = get_products_by_model(get_query_var('s'));
    } else {
	    $products = get_products($args, $products_per_page, $offset);
    }
...
<table class="table table-striped table-hover">
    ...
    <?php $alt = ($alt)? "alt" : "" ; ?>
        <tr>
          ...
          <?php 
            $args2 = array('where' => 'd.groschopp_database_id = '.$product->type_id, 'limit' => 1);
            $product_url = pods('product_id', $args2);
            $url = $product_url->field('search_slug')."?id=".$product->id;
          ?>

          <td class="<?php echo $alt ?>"><a href="<?php echo $url ?>"><?php echo $product->ordering_number ?></a></td>
          ...
          <td class="<?php echo $alt ?> text-center hidden-xs hidden-print"><a href="<?php bloginfo('template_url'); ?>/quote-add.php?id=<?php echo $product->id ?>" class="quote quote-add">Add to Quote</a></td>
        </tr>
    <?php $alt = !$alt ?>
    <?php endforeach; ?>
    </tbody>
