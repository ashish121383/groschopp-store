<?php $headers = get_product_headers(get_field('type', $_GET['id'])) ?>

<!-- main -->

    <form method="post" id="modal" class="clearfix" action="<?php echo get_bloginfo('template_url') ?>/submit.php" enctype="multipart/form-data">
      <input type="hidden" name="id" value="<?php echo $_GET['id'] ?>">
      <input type="hidden" name="product_id" value="<?php echo get_the_title($_GET['id']) ?>">
      ...
      <div class="row">
      	...
      	<div class="col-sm-6">
          <h2>
          Model #<?php echo get_the_title($_GET['id']) ?>
      ...
      <div class="row">
          ...
          */ ?>
	          ...
	          <?php $options = get_product_options($header->id, get_field('type', $_GET['id'])); $count = 0; ?>
	          <?php foreach($options as $option): ?>

            <?php $exception[] = ($option->value == 1 && get_field('voltage', $_GET['id']) != 115)? true : false ; ?>
            <?php $exception[] = ($option->value == 2 && strpos(get_field('model', $_GET['id']), "FC") !== false)? true : false; ?>
            <?php $exception[] = ($option->value == 3 && strpos(get_field('model', $_GET['id']), "PM108") !== false)? true : false; ?>
            <?php $exception[] = ($option->value == 4 && substr(get_field('model', $_GET['id']), -1) == "i")? true : false ;  ?>
            <?php $exception[] = ($option->value == 5 && strpos(get_field('model', $_GET['id']), "PM8") === false)? true : false; ?>
            <?php $count = (!in_array(1, $exception))? $count + 1 : $count; ?>
            <?php unset($exception); endforeach ?>

            <?php if($count > 0): ?>
            <section class="options">
            	<header data-product-id="<?php echo get_field('type', $_GET['id']) ?>" data-image="<?php echo $header->custom_image ?>">
              ...
              <div class="option-content" style="display: none;">
	              <?php foreach(get_product_options($header->id, get_field('type', $_GET['id'])) as $option): ?>
                ...
                <input type="<?php echo ($option->form_element === 'R')? "radio" : "checkbox" ; ?>" name="<?php echo $header->label ?><?php echo ($option->form_element !== 'R')? "[]" : ""; ?>" value="<?php echo $option->label ?>"> <?php echo $option->label ?> <input type="text" name="<?php echo $option->label ?>_temp">
                <?php else: ?>
                <input type="<?php echo ($option->form_element === 'R')? "radio" : "checkbox" ; ?>" name="<?php echo $header->label ?><?php echo ($option->form_element !== 'R')? "[]" : ""; ?>" value="<?php echo $option->label ?>"> <?php echo $option->label ?><br/>
        ...
        <div class="col-sm-6 custom-images">
          <img src="<?php bloginfo('url') ?>/data/customize/<?php echo get_field('type', $_GET['id']) ?>/Motor.jpg" alt="">
      ...
      <footer class="modal-controls form-footer text-right">
        <input class="text-btn" type="button" value="Cancel" onclick="history.go(-1); return false;">
        <input class="text-btn green" id="uploadfiles" type="submit" value="Continue">
      </footer>
      ...
      <?php wp_nonce_field( 'custom-motor-' . $_GET['id'] ); ?>
    </form>